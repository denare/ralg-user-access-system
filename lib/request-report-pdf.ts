...
function drawHeader(doc: PDFKit.PDFDocument, report: RequestReportData) {
  ...
  // Meta row (single line, smaller font)
  doc.font("Helvetica").fontSize(6.5).fillColor("#334155");
  const metaLine1 = `Request Number: ${report.request.requestNumber}   |   Request Status: ${report.request.status}   |   Submission Date: ${formatDate(report.request.submittedAt)}   |   Report Date: ${formatDate(report.generatedAt)}`;
  doc.text(metaLine1, textX, ty, { width: textW, align: "center" });
  ty += 10;
...
function finalSignatureBlock(doc: PDFKit.PDFDocument, verificationHash: string) {
  ...
  // Verification text (no fake seal)
  const vx = margins.left;
  doc.font("Helvetica-Bold").fontSize(7.5).fillColor("#0b2239")
    .text("Document Verification Code:", vx, y + 8);
  doc.font("Helvetica").fontSize(7).fillColor("#334155")
    .text(verificationHash, vx, y + 20, { width: contentWidth });
  doc.font("Helvetica").fontSize(6.5).fillColor("#64748b")
    .text(
      ...
      vx, y + 32, { width: contentWidth, lineGap: 1.5 }
    );
  doc.y = y + 64;
}
...
export async function renderRequestReportPdf(report: RequestReportData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    // Section 5: HOD Approval (force new page)
    doc.addPage();
    doc.y = HEADER_H + 12;
    ...
    // Section 8: Audit (force new page)
