...
export function SignupForm({ departments }: { departments: string[] }) {
  ...
  return <>
    <form onSubmit={submit} className="grid min-w-0 gap-5 xl:grid-cols-2">
      ...
      <p className="text-xs text-slate-500 xl:col-span-2">Use at least eight characters with uppercase, lowercase, and a number.</p>
      ...
      {message ? <p className={`border p-3 text-sm xl:col-span-2 ${isError ? "border-red-300 bg-red-50 text-red-800" : "border-emerald-300 bg-emerald-50 text-emerald-800"}`}>{message}</p> : null}
      {!isError && message ? <p className="text-sm xl:col-span-2">Next step: <Link href="/login" className="font-bold text-brand-government hover:underline">go to Sign In</Link> after confirming your email.</p> : null}
      <button disabled={submitting} className="button-primary xl:col-span-2">{submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <UserPlus className="h-4 w-4" />}{submitting ? "Creating Account..." : "Create Applicant Account"}</button>
    </form>
...
function Field({ label, wide, error, children }: { label: string; wide?: boolean; error?: string; children: ReactNode }) {
  return <label className={wide ? "xl:col-span-2" : undefined}>
